...
//让页面的高度最低是内容的总高度
var body_height = document.documentElement
  ? document.documentElement.clientHeight
  : document.body.clientHeight;
$(".up-page-wrapper").css("min-height", body_height);
$(".toTop").click(function () {
  /*点击回到顶部*/ $(".footer").fadeOut("slow", function () {
    //底部消失
...
var if250T = true /*scrool<250*/,
  if250F = true; /*scrool>250*/
...
window.onscroll = function () {
  ...
  if (document.getElementsByClassName("video-main")[0]) {
    /*首页--当滚动到视频教程部分--执行动画*/
    if (
      document.documentElement.clientHeight + $(document).scrollTop() - 180 >
      $(".video-main")[0].offsetTop
    ) {
...
/*这个是总的方法类*/
var all_function = {
  signature: function (_jsapi_ticket) {
    //传来_jsapi_ticket
  ...
  wxFenxiang: function () {
    //、、下面的是微信分享功能、、、、、、、、、、、、、、、、、、、、、、、、、、、、
    var _urlWX =
      hostAjax1 + "/InterfaceUrl/WeiXin/WeiXinService.asmx/WeiXinApi";
    var ajaxData = {};
    //判断token的时间--2小时
    if (localStorage.getItem("token_time")) {
      if (
        new Date() -
          new Date(JSON.parse(localStorage.getItem("token_time"))["time"]) >
        7200000
      ) {
    ...
    function callback(data) {
      ...
      postAjax(
        hostAjax1 + "/InterfaceUrl/WeiXin/WeiXinService.asmx/GetTicket",
        JSON.stringify({ access_token: JSON.parse(data.d).access_token }),
        callback2
      );
    }
  ...
  wxOpenId: function (_code) {
    //、、微信--登录--获取用户信息和openid、、、、、、、、、、、、、、、、、、、、、、、、、、、、
    getAjax(
      hostAjax + "/api/weixin/v1/access_token?code=" + _code,
      "",
      wxOpenIdCallback
    );
    //判断token的时间--2小时
    function wxOpenIdCallback(data) {
      if (data.Code == 0) {
        ...
        getAjax(
          hostAjax +
            "/api/weixin/v1/userinfo?access_token=" +
            JSON.parse(data.Data).access_token +
            "&openid=" +
            JSON.parse(data.Data).openid,
          "",
          openid
        ); //此接口为获取用户信息的接口
        //openId登录
        getAjax(
          hostAjax +
            "/api/user/v1/wxloginopenid?openid=" +
            JSON.parse(data.Data).openid +
            "&imgurl=" +
            JSON.parse(data.Data).imgurl +
            "&nickname=" +
            JSON.parse(data.Data).nickname +
            "&unionid=" +
            JSON.parse(data.Data).unionid,
          "",
          function () {}
        );
  ...
  countTime: function (id, times) {
    ...
    timer = setInterval(function () {
      ...
      if (times <= 0) {
        ...
        postAjax(
          hostAjax + "/Interface/PKService/PKService.asmx/ServiceUpdataPKState",
          JSON.stringify({ sPkID: id, iPkstate: 3 }),
          callback
        ); //0是未开始,1是让他进行,2是让他结束,3是让他无效
        function callback(data) {
          if (data.d.Code == 0) {
    ...
    timer = setInterval(function () {
      ...
      if (times <= 0) {
        ...
        if (localStorage.getItem("Pkstate") == 1) {
          postAjax(
            hostAjax +
              "/Interface/PKService/PKService.asmx/ServiceUpdataPKState",
            JSON.stringify({
              sPkID: localStorage.getItem("pkId"),
              iPkstate: 2,
            }),
            callback
          );
  ...
  vote: function (_sID, _iUserId, _iUserIdVote, _iPkstate) {
    var vote_url =
      hostAjax2 + "/Interface/PKService/PKService.asmx/ServiceAddVote";
    var vote_str = {
      sPkid: _sID,
      iUserId: parseInt(_iUserId),
      iUserIdVote: _iUserIdVote,
      iPktype: _iPkstate,
    };
  ...
  showpk_1v1: function () {
    ...
    var _url =
      hostAjax2 +
      "/Interface/PKService/PKService.asmx/ServiceSelectPKhomePage1V1";
    var ajaxData_str;
    if (localStorage.getItem("zhizhao_uid")) {
      ajaxData_str = {
        sID: _1v1_pkId,
        sTitle: "",
        iPkstate: Pkstate,
        iUserId: parseInt(localStorage.getItem("zhizhao_uid")),
      };
    } else {
      ajaxData_str = {
        sID: _1v1_pkId,
    ...
    function callback(data) {
      ...
      if (data.d.Data[0].pkstate == 2) {
        ...
        all_function.countDown(
          ".pk_time span",
          (new Date(all_function.iponeTime(data.d.Data[0].endtime)) -
            new Date()) /
            1000
        ); //倒计时
      }
        ...
        $(".pk_time button").click(function () {
          if (all_function.isLogin()) {
            ...
            if (if_guanzhu && $(this).text() == "关注PK") {
              all_function.guanzhu(
                _1v1_pkId,
                parseInt(localStorage.getItem("zhizhao_uid"))
              );
            }
            if_guanzhu = false;
          }
    ...
    var _url1 =
      hostAjax2 +
      "/Interface/PKService/PKService.asmx/ServiceSelectPKChallenger";
    var ajaxData_str1 = {
      sID: _1v1_pkId,
      sTitle: "",
      iPktype: 1,
      iSenduserId: 0,
      sSenduserNeme: "",
    }; /*0.未开始,1,进行中,2.结束 1：1v1,2：多人pk，3.冠军争霸*/
    ...
    function callback1(data) {
      ...
      $(".blue").html(
        "<div class='pk_name'>" +
          ...
          "</div><img src='" +
          img_url +
          data.d.Data[0].headimg +
          "'/><div class='adress'>" +
          data.d.Data[0].schoolneme +
          "</div>"
      );
      $(".blue img").click(function () {
        location.href =
          "person_page/person_detail.html?id=" + data.d.Data[0].senduserid;
      });
      //图片
      $(".header li")
        .eq(0)
      ...
      $(".header li:first-child")
        .find("img")
        .eq(0)
        .attr("src", all_function.firstImg(data.d.Data[0].uploadpath));
      $(".pk_information li:first-child")
        .find(".pk_inf_list2 img")
        .attr("src", all_function.firstImg(data.d.Data[0].narrowing));
      //票数
      $(".pk_information li:first-child")
        .find(".pk_inf_list1 p")
        .eq(0)
        .find("span")
        .text(data.d.Data[0].Voters);
      //已卖出
      $(".pk_information li:first-child")
        .find(".pk_inf_list1 p")
        .eq(1)
        .find("span")
        .text(data.d.Data[0].sellingtimes);
      //简介
      $(".pk_information li:first-child")
        .find(".pk_inf_list3 p")
        .text(data.d.Data[0].remark);
      //专家点评
      $(".pk_information li:first-child")
        .find(".pk_inf_list4 p")
        .text(data.d.Data[0].Commentcomments);
      //投票和购买的buttonid--擂主id
      $(".pk_information li:first-child button")
        .eq(0)
        .attr("index", data.d.Data[0].senduserid);
      $(".pk_information li:first-child button")
        .eq(0)
        .click(function () {
          //这是投票
    ...
    var _url2 =
      hostAjax2 +
      "/Interface/PKService/PKService.asmx/ServiceSelectContestantInformation";
    var ajaxData_str2 = {
      sID: _1v1_pkId,
      sTitle: "",
      iPktype: 1,
      ...
      sSenduserNeme: "",
      iUserId: uid,
    }; /*0.未开始,1,进行中,2.结束 1：1v1,2：多人pk，3.冠军争霸iUserId是用户id*/
    ...
    function callback2(data) {
      ...
      $(".pk_information li:last-child")
        ...
        .text(data.d.Data[0].Commentcomments);
      //投票和购买的buttonid--擂主id
      $(".pk_information li:last-child button")
        .eq(0)
        .attr("index", data.d.Data[0].senduserid);
      $(".pk_information li:last-child button")
        .eq(0)
  ...
  pkComment: function () {
    //调取评论信息
    var _url3 =
      hostAjax2 + "/Interface/PKService/PKService.asmx/ServiceSelectComment";
      ...
      if (data.d.Code == 0) {
        ...
        for (var i = 0; i < data.d.Data.length && i <= 2; i++) {
          ...
          commentStr +=
            ...
            commentImg +
            "'/><div class='name'>&ensp;" +
            data.d.Data[i].userName +
            "</div><p>" +
            data.d.Data[i].content +
            "</p></div>";
        }
        $(".comment>header span").text("共" + data.d.Data.length + "条评论");
      } else {
        commentStr =
          "<p style='text-align: center;font-size: 0.26rem;color:white;'>暂时没有评论哦~~</p>";
  ...
  guanzhu: function (_sPkid, _iUserId) {
    //主题id--登录人的id
    //所有的地方点击关注
    var guanzhu_url =
      hostAjax2 + "/Interface/PKService/PKService.asmx/ServiceAddReviewGuZhu";
  ...
  commit: function (_sPkid, _iCommentatorId, str) {
    var commit_url =
      hostAjax2 + "/Interface/PKService/PKService.asmx/ServiceAddReview";
    var commit_str = {
      sPkid: _sPkid,
      iCommentatorId: parseInt(_iCommentatorId),
      sContent: str,
    };
  ...
  yanzheng: function () {
    ...
    if (
      $("#passd").val() == "" ||
      !/^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{6,20}$/.test($("#passd").val())
    ) {
  ...
  //收藏
  shoucang: function (_user_id, _model_id, _type) {
    //1:收藏 2:取消收藏
    ajaxData1 = {
      /*访问后端接口--传递的参数---总模型列表的接口参数*/
      userid: parseInt(_user_id),
  ...
  mvm_time: function (a, b) {
    //a是pk的类型,b是pk的状态
    _url2 =
      hostAjax2 + "/Interface/PKService/PKService.asmx/ServiceSelectTheme";
    ajaxData_str2 = {
      sID: localStorage.getItem("pkId"),
      sTitle: "",
      iPktype: a,
      iPkstate: b,
    }; /*0.未开始,1,进行中,2.结束*/
...
if (document.getElementsByClassName("moreLike")[0]) {
  ...
  function callback1(data) {
    ...
    for (var i = 0; i < data.Data.list.length; i++) {
      moreLike_str +=
        "<li  id='" +
        data.Data.list[i].modelid +
        "'><img src='" +
        data.Data.list[i].smallimg +
        "'/><p>" +
        data.Data.list[i].modelname +
        "</p></li>";
    }
...
if (
  location.href.indexOf("senduserid") > 0 &&
  localStorage.getItem("senduserid") == null
) {
  localStorage.setItem(
    "senduserid",
    location.href.split("senduserid=")[1].split("&")[0]
  );
}
if (
  location.href.indexOf("inviteCode") > 0 &&
  sessionStorage.getItem("zhizhao_inviteCode") == null
) {
  sessionStorage.setItem(
    "zhizhao_inviteCode",
    location.href.split("inviteCode=")[1].split("&")[0]
  );
}
if (
  location.href.indexOf("code=") > 0 &&
  localStorage.getItem("zhizhao_openid") == null
) {
  //获取用户openid
  sessionStorage.setItem(
    "zhizhao_code",
...
function wxLogin(_url) {
  ...
  if (!isDeveloplment) {
    if (
      sessionStorage.getItem("zhizhao_code") == null &&
      localStorage.getItem("zhizhao_openizhizhao_openidd") == null
    ) {
      var wxUrl = encodeURI(location.href);
      location.href =
        "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx220f1ff8e48f8edb&redirect_uri=" +
        wxUrl +
        "&response_type=code&scope=snsapi_userinfo&state=jin#wechat_redirect";
    }
  }
}
