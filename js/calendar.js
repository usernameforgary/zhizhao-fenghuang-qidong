...
//		var beginDayOfWeek = new Date( new Date().getFullYear(), new Date().getMonth(), 1).getDay();
//		var daysOfMonth = new Date( new Date().getFullYear(), new Date().getMonth() + 1, 0).getDate();
//		console.log(daysOfMonth);
		if( curYMD == selectYMD ){
			
			spans[i].className = this.curDayClass+" "+"normal";
			this.currentIndex=i;
		}else if(spans[i].innerHTML!="&nbsp;"){
			spans[i].className = "normal";
	...
	if (getNowMonth==localMonth) {
		...
		if ((parseInt(sign.split("|")[0])+localDay-_daysOfMonth)>0) {
			...
			for (var i=nextBeginDayOfWeek;i<=nextBeginDayOfWeek+a-2;i++) {
				var currentSpan=this.allSpans[i];
				currentSpan.className="thisDay";
			}
		}
		
		var giftSpan=cr.allSpans[this.thisIndex];
...
//		if (this.Today==1) {
//			var sign=localStorage.getItem("sign");
//			var _beginDayOfWeek = new Date( this.Year, this.Month, 1).getDay();
//
//			window.localStorage.setItem("sign",sign.split("|")[0]+"|"+_beginDayOfWeek);
//		}
...
//点击--确定礼物的那一天（如果有数据）不用--如果没有数据（开始确定）（有数据，但是不连续--开始确定）
//	var selectYear  = this.find( this.caleTop.sq_year_id ).value;
//	var selectMonth = this.find( this.caleTop.sq_month_id ).value;
//	var daysOfMonthArray = this._getMonthViewArray( selectYear, selectMonth );
...
//	console.log(this.Month+"--"+curMonth)
	...
	var _beginDayOfWeek = new Date( this.Year, this.Month+1, 1).getDay();

		if (this.Year==curYear&&this.Month==curMonth-1) {
			if(localStorage.getItem("sign")){
					var localYear=parseInt(sign.split("|")[1].split("-")[0]);
		var localMonth=parseInt(sign.split("-")[1]);
		var localDay=parseInt(sign.split("-")[2]);
	console.log(beginDayOfWeek+"-----"+daysOfMonth+"----"+_beginDayOfWeek);
	var _daysOfMonth = new Date( localYear,localMonth, 0).getDate();	
				var currentSpan=this.allSpans[this.currentIndex];
				if (currentSpan.className=="thisDay") {
					alert("今天已经签到了！")
					console.log(parseInt(sign.split("|")[0])+localDay-_daysOfMonth)
				}else{
					if ((parseInt(sign.split("|")[0])+localDay-_daysOfMonth)<this.Today) {
						$(".giftDay").get(0).className="normal";
						alert("由于您没有连续签到，所以刷新签到了！")
						...
						currentSpan.className="thisDay";
					var _continueDay=parseInt(sign.split("|")[0]);
					_continueDay++;
						window.localStorage.setItem("sign",_continueDay+"|"+sign.split("|")[1]);
					}
				}
			}else{
				//如果location不存在--需要储存
				...
				currentSpan.className="thisDay";
				if (this.currentIndex+6>beginDayOfWeek+daysOfMonth) {
				this.thisIndex=this.currentIndex+6-(beginDayOfWeek+daysOfMonth)+_beginDayOfWeek;
				console.log(this.currentIndex+"-----"+this.thisIndex);
				
				}else{
					var giftSpan=this.allSpans[this.currentIndex+6];
					if (giftSpan.innerHTML!="&nbsp;") {
						giftSpan.className="giftDay";
					}
				}
				//第一次储存--连续的天数加年月日
				console.log(this.Year+"-"+(this.Month+1)+"-"+this.Today);
				window.localStorage.setItem("sign",1+"|"+this.Year+"-"+(this.Month+1)+"-"+this.Today);
			}
			...
			this._bindAllData( this.Year, this.Month+1 );
			cr.goPrevOrNextMonth(cr); 
