...
//	   	 getAjax(hostAjax+"/api/weixin/v1/userinfo?access_token="+JSON.parse(data.Data).access_token+"&openid="+JSON.parse(data.Data).openid,"",openid);//此接口为获取用户信息的接口
	   	 	//openId登录
//	   	 	getAjax(hostAjax+"/api/user/v1/wxloginopenid?openid="+JSON.parse(data.Data).openid,"",relogin);
...
// 		//下面是微信授权
// 		if(sessionStorage.getItem("zhizhao_code")==null&&localStorage.getItem("zhizhao_openid")==null){
